...
struct Vector2D{
	...
	inline Vector2D operator*(float scalar){	return Vector2D(x * scalar, y * scalar);}
	...
	inline bool operator==(const Vector2D& rhs) const{	return (x == rhs.x)	&& (y == rhs.y);}
	...
	static inline float Dot(const Vector2D& v1, const Vector2D& v2)	{	return v1.x * v2.x + v1.y * v2.y;	}
